...
def register_blueprint_tools(mcp: FastMCP):
    ...
    @mcp.tool()
    def set_component_material_color(
        ctx: Context,
        blueprint_name: str,
        component_name: str,
        ...
        """
        Set a color on a StaticMeshComponent's material in a Blueprint.
        Creates a dynamic material instance with the specified color.
        ...
        try:
            ...
            logger.info(f"Setting component material color with params: {params}")
            response = unreal.send_command("set_component_material_color", params)

            if not response:
                logger.error("No response from Unreal Engine")
                return {"success": False, "message": "No response from Unreal Engine"}

            logger.info(f"Set component material color response: {response}")
            return response

        except Exception as e:
            error_msg = f"Error setting component material color: {e}"
            logger.error(error_msg)
            return {"success": False, "message": error_msg}
    ...
    def set_blueprint_variable_default(
        ...
        Args:
            ...
            default_value: Default value as a string (e.g., "100", "3.14", "true", "Hello")
        ...
        try:
            ...
            logger.info(f"Setting blueprint variable default with params: {params}")
            response = unreal.send_command("set_blueprint_variable_default", params)

            if not response:
                logger.error("No response from Unreal Engine")
                return {"success": False, "message": "No response from Unreal Engine"}

            logger.info(f"Set blueprint variable default response: {response}")
            return response

        except Exception as e:
            error_msg = f"Error setting blueprint variable default: {e}"
            logger.error(error_msg)
            return {"success": False, "message": error_msg}
    ...
    @mcp.tool()
    def create_character_blueprint(ctx: Context, name: str, save_path: str = "/Game/Blueprints") -> Dict[str, Any]:
        """Create a new Blueprint subclassing ACharacter with CapsuleComponent, CharacterMovement, and SkeletalMesh auto-created.

        Args:
            name: Name for the new character Blueprint.
            save_path: Content browser folder. Defaults to /Game/Blueprints.
        """
        from unreal_mcp_server import get_unreal_connection
        try:
            unreal = get_unreal_connection()
            if not unreal:
                return {"success": False, "message": "Failed to connect to Unreal Engine"}
            response = unreal.send_command("create_character_blueprint", {"name": name, "save_path": save_path})
            return response or {}
        except Exception as e:
            logger.error(f"Error creating character blueprint: {e}")
            return {"success": False, "message": str(e)}

    @mcp.tool()
    def assign_anim_blueprint(ctx: Context, character_bp: str, anim_bp_path: str) -> Dict[str, Any]:
        """Assign an Animation Blueprint to a Character Blueprint's SkeletalMeshComponent.

        Args:
            character_bp: Name of the character Blueprint.
            anim_bp_path: Asset path to the Animation Blueprint.
        """
        from unreal_mcp_server import get_unreal_connection
        try:
            unreal = get_unreal_connection()
            if not unreal:
                return {"success": False, "message": "Failed to connect to Unreal Engine"}
            response = unreal.send_command("assign_anim_blueprint", {"character_bp": character_bp, "anim_bp_path": anim_bp_path})
            return response or {}
        except Exception as e:
            logger.error(f"Error assigning anim blueprint: {e}")
            return {"success": False, "message": str(e)}

    @mcp.tool()
    def set_movement_param(ctx: Context, character_bp: str, param_name: str, value: float) -> Dict[str, Any]:
        """Set a CharacterMovementComponent property on a Character Blueprint.

        Common params: MaxWalkSpeed, JumpZVelocity, AirControl, GravityScale, MaxAcceleration, BrakingDecelerationWalking.

        Args:
            character_bp: Name of the character Blueprint.
            param_name: Property name on UCharacterMovementComponent.
        ...
        try:
            ...
            if not unreal:
                return {"success": False, "message": "Failed to connect to Unreal Engine"}
            response = unreal.send_command("set_movement_param", {"character_bp": character_bp, "param_name": param_name, "value": float(value)})
            return response or {}
        except Exception as e:
            logger.error(f"Error setting movement param: {e}")
            return {"success": False, "message": str(e)}
