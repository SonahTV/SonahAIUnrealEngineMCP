...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleCommand(const FString& CommandType, const TSharedPtr<FJsonObject>& Params)
{
    ...
    // Stage 5: Character domain
    else if (CommandType == TEXT("create_character_blueprint"))
    {
        return HandleCreateCharacterBlueprint(Params);
    }
    else if (CommandType == TEXT("assign_anim_blueprint"))
    {
        return HandleAssignAnimBlueprint(Params);
    }
    else if (CommandType == TEXT("set_movement_param"))
    {
        return HandleSetMovementParam(Params);
    }

    else if (CommandType == TEXT("set_blueprint_variable_default"))
    {
        FString BPName, VarName, Value;
        ...
        if (!bFound) return FUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Variable not found"));
        FBlueprintEditorUtils::MarkBlueprintAsModified(BP);
        TSharedPtr<FJsonObject> R = MakeShared<FJsonObject>();
        R->SetStringField(TEXT("variable"), VarName);
        R->SetStringField(TEXT("default_value"), Value);
        return R;
    }
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleAddComponentToBlueprint(const TSharedPtr<FJsonObject>& Params)
{
    ...
    ComponentClass = FindObject<UClass>(static_cast<UObject*>(nullptr), *ComponentType);
    
    // If not found, try with "Component" suffix
    if (!ComponentClass && !ComponentType.EndsWith(TEXT("Component")))
    {
        FString ComponentTypeWithSuffix = ComponentType + TEXT("Component");
        ComponentClass = FindObject<UClass>(static_cast<UObject*>(nullptr), *ComponentTypeWithSuffix);
    }
    ...
    if (!ComponentClass && !ComponentType.StartsWith(TEXT("U")))
    {
        ...
        ComponentClass = FindObject<UClass>(static_cast<UObject*>(nullptr), *ComponentTypeWithPrefix);
        
        // Try with both prefix and suffix
        if (!ComponentClass && !ComponentType.EndsWith(TEXT("Component")))
        {
            FString ComponentTypeWithBoth = TEXT("U") + ComponentType + TEXT("Component");
            ComponentClass = FindObject<UClass>(static_cast<UObject*>(nullptr), *ComponentTypeWithBoth);
        }
    }
...
// ============================================================================
// Stage 5: Character domain handlers
// ============================================================================

TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleCreateCharacterBlueprint(const TSharedPtr<FJsonObject>& Params)
{
    FString Name;
    ...
    // Create a Blueprint subclassing ACharacter
    ...
    Result->SetStringField(TEXT("blueprint_path"), BP->GetPathName());
    Result->SetStringField(TEXT("parent_class"), TEXT("Character"));

    // List the auto-created components from ACharacter
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleAssignAnimBlueprint(const TSharedPtr<FJsonObject>& Params)
{
    FString CharacterBPName;
    if (!Params->TryGetStringField(TEXT("character_bp"), CharacterBPName))
    {
        return FUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Missing 'character_bp' parameter"));
    }

    FString AnimBPPath;
    if (!Params->TryGetStringField(TEXT("anim_bp_path"), AnimBPPath))
    {
        return FUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Missing 'anim_bp_path' parameter"));
    }

    UBlueprint* BP = FUnrealMCPCommonUtils::FindBlueprint(CharacterBPName);
    if (!BP)
    {
        return FUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Character BP not found: %s"), *CharacterBPName));
    }
    ...
    if (!AnimBP)
    {
        return FUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Failed to load AnimBP: %s"), *AnimBPPath));
    }

    // Find the SkeletalMeshComponent in the character BP's SCS
    ...
    if (BP->SimpleConstructionScript)
    {
        for (USCS_Node* Node : BP->SimpleConstructionScript->GetAllNodes())
        {
            ...
            {
                USkeletalMeshComponent* SMC = Cast<USkeletalMeshComponent>(Node->ComponentTemplate);
                if (SMC)
                {
                    MeshComp = SMC;
                    break;
    ...
    // Also check the CDO for the inherited mesh component
    if (!MeshComp && BP->GeneratedClass)
    {
        if (ACharacter* CDO = Cast<ACharacter>(BP->GeneratedClass->GetDefaultObject()))
        {
            MeshComp = CDO->GetMesh();
        }
    }

    if (!MeshComp)
    {
        return FUnrealMCPCommonUtils::CreateErrorResponse(TEXT("No SkeletalMeshComponent found on character BP"));
    }
    ...
    TSharedPtr<FJsonObject> Result = MakeShared<FJsonObject>();
    Result->SetStringField(TEXT("character_bp"), CharacterBPName);
    Result->SetStringField(TEXT("anim_bp_path"), AnimBPPath);
    Result->SetStringField(TEXT("assigned_to_component"), MeshComp->GetName());
    return Result;
}

TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleSetMovementParam(const TSharedPtr<FJsonObject>& Params)
{
    FString CharacterBPName;
    if (!Params->TryGetStringField(TEXT("character_bp"), CharacterBPName))
    {
    ...
    // Find CharacterMovementComponent in SCS or CDO
    ...
    if (BP->SimpleConstructionScript)
    {
        for (USCS_Node* Node : BP->SimpleConstructionScript->GetAllNodes())
        {
            if (Node && Node->ComponentTemplate)
            {
                UCharacterMovementComponent* CMC = Cast<UCharacterMovementComponent>(Node->ComponentTemplate);
                if (CMC)
                {
                    MoveComp = CMC;
                    break;
    ...
    if (!MoveComp && BP->GeneratedClass)
    {
        if (ACharacter* CDO = Cast<ACharacter>(BP->GeneratedClass->GetDefaultObject()))
        {
            MoveComp = CDO->GetCharacterMovement();
        }
    }

    if (!MoveComp)
    {
        return FUnrealMCPCommonUtils::CreateErrorResponse(TEXT("No CharacterMovementComponent found on character BP"));
    }

    // Set the property by name using UE reflection
    FProperty* Prop = FindFProperty<FProperty>(UCharacterMovementComponent::StaticClass(), *ParamName);
    if (!Prop)
    {
        return FUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Property not found on CharacterMovementComponent: %s"), *ParamName));
    }

    // Handle float/double property
    if (FFloatProperty* FloatProp = CastField<FFloatProperty>(Prop))
    {
        FloatProp->SetPropertyValue_InContainer(MoveComp, static_cast<float>(Value));
    }
    else if (FDoubleProperty* DoubleProp = CastField<FDoubleProperty>(Prop))
    {
        DoubleProp->SetPropertyValue_InContainer(MoveComp, Value);
    }
    else if (FIntProperty* IntProp = CastField<FIntProperty>(Prop))
    {
        IntProp->SetPropertyValue_InContainer(MoveComp, static_cast<int32>(Value));
    }
    else if (FBoolProperty* BoolProp = CastField<FBoolProperty>(Prop))
    {
        BoolProp->SetPropertyValue_InContainer(MoveComp, Value != 0.0);
    }
    else
    {
        return FUnrealMCPCommonUtils::CreateErrorResponse(FString::Printf(TEXT("Unsupported property type for %s"), *ParamName));
    }
