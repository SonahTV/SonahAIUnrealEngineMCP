...
{
    // If it's already a full path (starts with /), use it directly
    if (BlueprintName.StartsWith(TEXT("/")))
    {
        // Try with .AssetName suffix first (LoadObject needs /Path/Name.Name)
        FString FullPath = BlueprintName;
        if (!FullPath.Contains(TEXT(".")))
        {
            FString BaseName = FPackageName::GetShortName(FullPath);
            FullPath = FullPath + TEXT(".") + BaseName;
        }
        UBlueprint* BP = LoadObject<UBlueprint>(nullptr, *FullPath);
        if (BP) return BP;
    }

    // Try default prefix /Game/Blueprints/
    FString AssetPath = TEXT("/Game/Blueprints/") + BlueprintName + TEXT(".") + BlueprintName;
    UBlueprint* BP = LoadObject<UBlueprint>(nullptr, *AssetPath);
    if (BP) return BP;

    // Search asset registry by name as last resort
    IAssetRegistry& AR = FModuleManager::LoadModuleChecked<FAssetRegistryModule>("AssetRegistry").Get();
    TArray<FAssetData> Assets;
    AR.GetAssetsByClass(UBlueprint::StaticClass()->GetClassPathName(), Assets);
    for (const FAssetData& AD : Assets)
    {
        if (AD.AssetName.ToString().Equals(BlueprintName, ESearchCase::IgnoreCase))
        {
            return Cast<UBlueprint>(AD.GetAsset());
        }
    }
...
// ============================================================================
// Stage 2: Blueprint introspection serializers
// ============================================================================

TSharedPtr<FJsonObject> FUnrealMCPCommonUtils::SerializePinToJson(UEdGraphPin* Pin)
{
    ...
    for (UEdGraphPin* Other : Pin->LinkedTo)
    {
        if (Other && Other->GetOwningNode())
        {
            ...
            Link->SetStringField(TEXT("node_id"), Other->GetOwningNode()->NodeGuid.ToString());
            Link->SetStringField(TEXT("pin_name"), Other->PinName.ToString());
...
TSharedPtr<FJsonObject> FUnrealMCPCommonUtils::SerializeNodeToJson(UEdGraphNode* Node, bool bIncludePins)
{
    ...
    if (UK2Node_CallFunction* CallFn = Cast<UK2Node_CallFunction>(Node))
    {
        ...
        {
            NodeObj->SetStringField(TEXT("function"), Fn->GetName());
            if (UClass* OwnerClass = Fn->GetOwnerClass())
            {
                NodeObj->SetStringField(TEXT("function_owner"), OwnerClass->GetPathName());
            }
        }
    }
    else if (UK2Node_VariableGet* VarGet = Cast<UK2Node_VariableGet>(Node))
    {
        NodeObj->SetStringField(TEXT("variable"), VarGet->GetVarNameString());
        NodeObj->SetStringField(TEXT("variable_op"), TEXT("get"));
    }
    else if (UK2Node_VariableSet* VarSet = Cast<UK2Node_VariableSet>(Node))
    {
        NodeObj->SetStringField(TEXT("variable"), VarSet->GetVarNameString());
        NodeObj->SetStringField(TEXT("variable_op"), TEXT("set"));
    }
    else if (UK2Node_Event* Evt = Cast<UK2Node_Event>(Node))
    {
        NodeObj->SetStringField(TEXT("event_name"), Evt->EventReference.GetMemberName().ToString());
    }
    ...
    if (bIncludePins)
    {
        ...
        for (UEdGraphPin* Pin : Node->Pins)
        {
            PinArr.Add(MakeShared<FJsonValueObject>(SerializePinToJson(Pin)));
        }
...
TSharedPtr<FJsonObject> FUnrealMCPCommonUtils::SerializeGraphToJson(UEdGraph* Graph)
{
    ...
    for (UEdGraphNode* Node : Graph->Nodes)
    {
        NodeArr.Add(MakeShared<FJsonValueObject>(SerializeNodeToJson(Node, true)));
    }
